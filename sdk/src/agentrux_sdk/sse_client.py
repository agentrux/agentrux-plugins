...
SSOT: docs/04_design/sdk/sdk_design.md §5-2, docs/04_design/messaging/read_flow.md §9-C,
ADR-0002 (at-least-once)

server SSE は hint-only (read_flow.md §9-C-4): `event: hint` frame は
{topic_id, event_id, seq, event_type, payload_kind, stored_at} のみで payload 本体を
含まない。SDK は sdk_design.md §5-2 の「read_sse は full Event を yield」契約を守るため、
hint を受けるたびに GET /events/{evt_id} で本体を hydrate して Event を yield する。
"""
...
async def read_sse(
    ...
    *,
    ...
    hydrate する。`event: error` は SDK 例外に map して raise、`event: resync_required` は
    接続を維持したまま Pull で再同期 (read_flow.md ADR-0002)。

    Last-Event-ID header (HTTP 標準 SSE) で replay 開始位置を指示。cursor は受信した
    SSE `id:` を権威とし、hydration の成否に依存しない (取りこぼし / 同一 hint 再 replay 防止)。
        ...
        async with client.http._client.stream(
            ...
            async for event_type, data, frame_id in _iter_sse_frames(r):
                if event_type == "hint":
                    ...
                    if not event_id:
                        continue
                    # cursor は hint の event_id (= SSE id: 行、 server 実装上一致) を権威に進める。
                    # error/heartbeat 等 非 hint frame の id: (例 `id: -`) で汚染しない (再接続の
                    ...
                    # ADR-0002: 接続を維持したまま Pull で再同期し、同一接続上の次の hint を待つ。
                    # server の next_action は retention_miss / replay_gap とも "pull_resync"。
                    # 未知の next_action は frame を無視して接続維持 (投機的処理をしない)。
                    if data.get("next_action") not in (None, "pull_resync"):
                        continue
                    # resume cursor は server の details.resume_via が示す位置だが、 SSOT
                    # (read_flow.md §9-C: retention_miss=`<consumer_known_cursor>`,
                    # replay_gap=`after=evt_<requested_evt_id>`) 上いずれも consumer 自身が送った
                    # Last-Event-ID = cursor.value に一致する。 URL parse を増やさず cursor.value を使う。
                    # evicted でも read_pull が cursor_advance (oldest_available) を吸収して raise
                    # しない。 on_cursor_advance=cursor.set で 0 件再同期でも reconnect cursor を最新化。
