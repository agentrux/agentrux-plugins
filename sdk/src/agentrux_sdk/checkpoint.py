...
class CheckpointStore(ABC):
    ...
    async def load(self, topic_id: str) -> str | None:
        """最後に commit した event_id を返す (なければ None)."""

    @abstractmethod
    async def commit(self, topic_id: str, event_id: str) -> None:
        """event_id を最新 checkpoint として永続化."""
...
class InMemoryCheckpointStore(CheckpointStore):
    ...
    async def commit(self, topic_id: str, event_id: str) -> None:
        self._cp[topic_id] = event_id


class FileCheckpointStore(CheckpointStore):
    """JSON ファイル 1 つに { topic_id: event_id } を保存。 1 process 専用 (file lock なし).
