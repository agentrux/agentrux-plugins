...
| **Trigger** (`agentrux-trigger`) | Trigger plugin | Topic への新着 event で workflow を起動する | [`dify-agentrux-trigger-1.0.8.difypkg`](./dify-agentrux-trigger-1.0.8.difypkg) |
...
2. [`dify-agentrux-trigger-1.0.8.difypkg`](./dify-agentrux-trigger-1.0.8.difypkg) をアップロード
...
- **Trigger ソース**: [`src-trigger-1.0.8/`](./src-trigger-1.0.8/) — `provider/agentrux_api.py` (runtime + ttl_expired 検出), `trigger/events/new_event/new_event.py` (cursor + ttl_expired 復帰 skip-to-latest), `trigger/sse_worker.py`
